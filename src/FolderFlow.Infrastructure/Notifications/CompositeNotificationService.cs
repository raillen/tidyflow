...
namespace FolderFlow.Infrastructure.Notifications;
...
public class CompositeNotificationService : IExternalNotificationService
{
    ...
    private async Task SendWebhookAsync(string url, string type, string jobName, bool success, int count, string? error)
    {
        try
        {
            ...
            {
                payload = new {
                    content = $"**FolderFlow**: {jobName} -> {(success ? "OK" : "FALHOU")}",
                    embeds = new[] {
                        new {
                            title = jobName,
            ...
            {
                payload = new {
                    text = $"*FolderFlow*: {jobName} status: {(success ? ":white_check_mark:" : ":x:")}\n{(success ? $"Arquivos: {count}" : $"Detalhes: {error}")}"
                };
            }
            else
            {
    ...
    private async Task SendEmailAsync(AppSettings s, string jobName, bool success, int count, string? error)
    {
        try
        {
            ...
            var mail = new MailMessage("no-reply@folderflow.com", s.NotificationEmail, $"FolderFlow: {jobName} - {(success ? "Sucesso" : "Falha")}", body);
